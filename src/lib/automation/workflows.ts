  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        ...
        "User-Agent": "FinAdvisor-CRM-Automations/1.0",
        "X-FinAdvisor-Source": "automation",
        "X-FinAdvisor-Workflow": workflowKey,
        "X-FinAdvisor-Trigger": input.trigger,
        ...(signature ? { "X-FinAdvisor-Signature": `sha256=${signature}` } : {}),
      },
      body,
