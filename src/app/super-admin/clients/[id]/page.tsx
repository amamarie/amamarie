...
import { SuperAdminHeader } from "@/components/super-admin/SuperAdminChrome"
import { PageIntro, SectionCard, StatusPill } from "@/components/developer/DeveloperChrome"
...
import {
  ...
  updateOrganizationSensitiveStatus,
      ...
      <section className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <nav className="mt-4 flex gap-2 overflow-x-auto rounded-2xl border border-slate-200 bg-white p-2 shadow-sm">
          {tabs.map(([key, label]) => (
            <Link
              key={key}
              href={`/super-admin/clients/${data.organization.id}?tab=${key}`}
...
function SubscriptionTab({ data }: { data: NonNullable<Awaited<ReturnType<typeof getSuperAdminAccount360>>> }) {
  return (
    <div className="mt-4 grid gap-4 xl:grid-cols-[1fr_360px]">
      ...
      <SectionCard title="Actions abonnement sensibles" eyebrow="Confirmation typée">
        <SensitiveSubscriptionActions organizationId={data.organization.id} organizationName={data.organization.name} />
      </SectionCard>
    </div>
  )
}
...
function SecurityTab({ data }: { data: NonNullable<Awaited<ReturnType<typeof getSuperAdminAccount360>>> }) {
  return (
    <div className="mt-4 grid gap-4 xl:grid-cols-[1fr_360px]">
      ...
      <SectionCard title="Verrouillage compte" eyebrow="Action critique">
        <SensitiveSubscriptionActions organizationId={data.organization.id} organizationName={data.organization.name} />
      </SectionCard>
    </div>
  )
...
function SensitiveSubscriptionActions({ organizationId, organizationName }: { organizationId: string; organizationName: string }) {
  return (
    <div className="mt-4 grid gap-3">
      <p className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm font-medium leading-6 text-amber-800">
        Ces actions modifient l’accès du cabinet {organizationName}. Elles sont historisées dans l’audit log et exigent une confirmation exacte.
      </p>
      <SensitiveStatusForm organizationId={organizationId} action="PAST_DUE" label="Marquer paiement requis" confirmation="IMPAYE" reasonPlaceholder="Facture impayée, carte expirée..." />
      <SensitiveStatusForm organizationId={organizationId} action="SUSPEND" label="Suspendre le compte" confirmation="SUSPENDRE" reasonPlaceholder="Motif de suspension" />
      <SensitiveStatusForm organizationId={organizationId} action="REACTIVATE" label="Réactiver le compte" confirmation="REACTIVER" reasonPlaceholder="Motif de réactivation" />
      <SensitiveStatusForm organizationId={organizationId} action="CANCEL" label="Annuler l’abonnement" confirmation="ANNULER" reasonPlaceholder="Motif d’annulation" />
    </div>
  )
}

function SensitiveStatusForm({ organizationId, action, label, confirmation, reasonPlaceholder }: { organizationId: string; action: string; label: string; confirmation: string; reasonPlaceholder: string }) {
  return (
    <form action={updateOrganizationSensitiveStatus} className="grid gap-2 rounded-xl border border-slate-200 bg-slate-50 p-3">
      <input type="hidden" name="organizationId" value={organizationId} />
      <input type="hidden" name="sensitiveAction" value={action} />
      <p className="text-sm font-semibold text-slate-950">{label}</p>
      <input name="reason" placeholder={reasonPlaceholder} className="rounded-lg border border-slate-200 px-3 py-2 text-sm" />
      <div className="grid gap-2 sm:grid-cols-[1fr_auto]">
        <input name="confirmation" placeholder={confirmation} className="rounded-lg border border-slate-200 px-3 py-2 text-sm" />
        <button className="rounded-lg bg-slate-950 px-3 py-2 text-sm font-semibold text-white">{label}</button>
      </div>
    </form>
  )
}
