...
export async function GET(_request: Request, { params }: RouteContext) {
  try {
    ...
    const records = await prisma.amlBeneficialOwnershipRecord.findMany({ where: { organizationId, clientId: id }, orderBy: { createdAt: "desc" } })
    return ok({ records })
  } catch (error) {
...
export async function POST(request: Request, { params }: RouteContext) {
  try {
    ...
    await createAuditLog({
      ...
      entityId: record.id,
      action: "AML_BENEFICIAL_OWNER_RECORDED",
      newValue: {
        ultimateBeneficialOwnerName: record.ultimateBeneficialOwnerName,
        directOwnershipPercentage: record.directOwnershipPercentage?.toString(),
        indirectOwnershipPercentage: record.indirectOwnershipPercentage?.toString(),
      },
      source: "advisor",
