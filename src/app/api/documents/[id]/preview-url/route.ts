...
export async function GET(request: Request, { params }: RouteContext) {
  try {
    ...
    if (!canPreviewMimeType(document.mimeType)) return fail("PREVIEW_UNAVAILABLE", "Prévisualisation non disponible pour ce type de fichier.", 400)
    const settings = await ensureDocumentVaultSettings(user.organizationId)
    if (!canAccessDocumentContent({ settings, role: user.role, document })) return fail("DOCUMENT_RESTRICTED", "Accès au contenu restreint selon la politique documentaire du cabinet.", 403)
    ...
    await logDocumentAccess({
      ...
      eventType: "PREVIEW",
      request,
      purpose: "Prévisualisation par utilisateur autorisé",
      metadata: { expiresIn: 300, mimeType: document.mimeType },
    })
    await createCrmActivity({ organizationId: user.organizationId, userId: user.id, clientId: document.clientId, leadId: document.leadId, type: "DOCUMENT_PREVIEWED", title: "Prévisualisation document", description: document.name })
